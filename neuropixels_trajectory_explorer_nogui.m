...
gui_data.probe_coordinates_text = annotation('textbox','String','No probe selected', ...
    'Units','normalized','Position',[0,0,1,1],'VerticalAlignment','top', ...
    'FontSize',12,'FontName','Consolas','PickableParts','none'); % Probe coordinates text
...
update_probe_position(gui_data);
...
function update_slice(gui_data,varargin)
...
gui_data = update_probe_position(gui_data);
gui_data = update_probe_areas_coordinates(gui_data);
% update_slice(gui_data);
...
gui_data = update_probe_areas_coordinates(probe_atlas_gui);
gui_data = update_slice(probe_atlas_gui);

end


function gui_data = update_probe_position(gui_data)
...
probe_rotation_rad = -deg2rad(gui_data.probe.angle(3));
...
shank_ref_vec = gui_data.probe.reference_vector;
...
ref_shank = gui_data.probe.ref_shank;
dv_target = gui_data.probe.dv;
...
for curr_shank = 1:size(shank_vector,2)
    gui_data.probe.line(curr_shank).XData = new_probe_position(:,curr_shank,1);
    gui_data.probe.line(curr_shank).YData = new_probe_position(:,curr_shank,2);
    gui_data.probe.line(curr_shank).ZData = new_probe_position(:,curr_shank,3);
end

% Update probe angles
gui_data.probe.angle(1:2) = ...
...
function gui_data = update_probe_areas_coordinates(gui_data,varargin)
...
probe_sample_points_bregma = reshape( ...
    interp1([0,gui_data.probe.length], ...
...
[gui_data.probe.insertion_point.XData, ...
    gui_data.probe.insertion_point.YData, ...
    gui_data.probe.insertion_point.ZData] = ...
...
probe_angle_text = sprintf('Angle:      %.0f%c azimuth, %.0f%cH/%.0f%cV elevation, %.0f%c rotation', ...
    gui_data.probe.angle(1),char(176), ...
    gui_data.probe.angle(2),char(176), ...
    90-gui_data.probe.angle(2),char(176), ...
    gui_data.probe.angle(3),char(176));
...
gui_data.probe_area_labels = probe_area_labels;
gui_data.probe_area_boundaries = probe_area_boundaries;
...
function gui_data = probe_add(~,~,gui_data,probe_type)
% Add probe
...
%% Might want to add this back in if you want to save the probe positions as a .mat
